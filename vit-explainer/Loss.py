...
class FocalLoss(nn.modules.loss._WeightedLoss):
    ...
    def forward(self, input_, target):
        ce_loss = F.cross_entropy(
            input_, target, reduction=self.reduction, weight=self.weight)
...
class BiTemperedLogisticLoss(_Loss):
    ...
    def compute_normalization_fixed_point(cls, activations, t, num_iters=5):
        ...
        while i < num_iters:
            ...
            logt_partition = torch.sum(
                cls.exp_t(normalized_activations, t), dim=-1).view(-1, 1)
            normalized_activations = normalized_activations_step_0 * \
    ...
    def forward(self, inputs, targets):
        ...
        temp1 = (self.log_t(targets + 1e-10, self.t1) -
                 self.log_t(probabilities, self.t1)) * targets
        temp2 = (1 / (2 - self.t1)) * (torch.pow(targets, 2 - self.t1) -
                                       torch.pow(probabilities, 2 - self.t1))
        loss = temp1 - temp2
        loss = loss.sum(dim=-1)
